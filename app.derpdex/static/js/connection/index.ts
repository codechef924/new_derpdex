...
import OKX_LOGO from 'assets/svg/okx-logo.svg'
...
import { getIsCoinbaseWallet, getIsInjected, getIsMetaMaskWallet, getIsOKXWallet } from './utils'
...
const [web3Network, web3NetworkHooks] = initializeConnector<Network>(
  (actions) => new Network({ actions, urlMap: RPC_PROVIDERS, defaultChainId: 324 /* 280 */ })
)
...
const OKX_WalletConnection: Connection = {
  getName: () => 'OKX Wallet',
  connector: web3OKXWallet,
  hooks: web3OKXWalletHooks,
  type: ConnectionType.OKX_WALLET,
  getIcon: () => OKX_LOGO,
  shouldDisplay: () => Boolean((isMobile && !getIsInjectedMobileBrowser()) || !isMobile || getIsOKXWalletBrowser()),
  // If on a mobile browser that isn't the okx wallet browser, deeplink to the okx wallet app
  overrideActivate: () => {
    if (isMobile && !getIsInjectedMobileBrowser()) {
      window.open('https://www.okx.com/download', '_blank')
      return true
    } else if (!isMobile && typeof window.okxwallet === 'undefined') {

...
import { SearchDerpDexToken } from './../components/NavBar/SuggestionRow'
...
export function useFetchedTokenDatas(tokenAddresses: string[]): {
  loading: boolean
  error: boolean
  data: SearchDerpDexToken[] | undefined
} {
  ...
  const formatted = tokenAddresses.reduce((accum: { [address: string]: SearchDerpDexToken }, address, index) => {

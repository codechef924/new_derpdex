...
export function useContractByChainId<T extends Contract = Contract>(
  ...
  return useMemo(() => {
    if (!addressOrAddressMap || !ABI || !RPC_PROVIDERS[chainId] || !chainId) return null
    let address: string | undefined
    if (typeof addressOrAddressMap === 'string') address = addressOrAddressMap
    else address = addressOrAddressMap[chainId]
    if (!address) return null
    try {
      return getContract(address, ABI, RPC_PROVIDERS[chainId], undefined)
    } catch (error) {
      console.error('Failed to get contract', error)
      return null
    }
  }, [addressOrAddressMap, ABI, chainId, withSignerIfPossible]) as T
}
...
export function useV3NFTPositionManagerContractByChainId(
  chainId: number,
  withSignerIfPossible?: boolean
): NonfungiblePositionManager | null {
  return useContractByChainId<NonfungiblePositionManager>(
    chainId,
    NONFUNGIBLE_POSITION_MANAGER_ADDRESSES,
    NFTPositionManagerABI,
    withSignerIfPossible

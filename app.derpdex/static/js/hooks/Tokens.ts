...
import { useUserAddedTokens, useUserAddedTokensByChainId, useUserAddedTokensOnChain } from '../state/user/hooks'
...
/** Returns all tokens from the default list + user added tokens */
export function useDefaultActiveTokensV2(chainId: Maybe<SupportedChainId>): { [address: string]: Token } {
  const defaultListTokens = useCombinedActiveList()
  const tokensFromMap = useTokensFromMap(defaultListTokens, chainId)
  const userAddedTokens = useUserAddedTokensByChainId(chainId)
  return useMemo(() => {
    return (
      userAddedTokens
        // reduce into all ALL_TOKENS filtered by the current chain
...
export function useTokenByDefinedChainId(tokenAddress: string | undefined, chainId: number): Token | null | undefined {
  const tokens = useDefaultActiveTokens(chainId)
  return useTokenFromMapOrNetwork(tokens, tokenAddress)
}
...
export function useCurrencyByChainId(
  ...
  const tokens = useDefaultActiveTokensV2(chainId)

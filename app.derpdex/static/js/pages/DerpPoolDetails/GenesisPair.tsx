...
const AddMoreBtn = styled.button`
  ...
  border-radius: 8px;
  background: #fff;
  background: linear-gradient(270deg, #46c9d2 0%, #a372ff 100%), linear-gradient(0deg, #ffffff, #ffffff);
  color: #fff;
  cursor: pointer;
`
...
export const GenesisPair = (props: PoolProps) => {
  ...
  return (
    <Container>
      ...
      <ButtonContainer>
        <AddMoreBtn onClick={() => navigate(`/add/${pool.token0.address}/${pool.token1.address}/${pool.feeTier}`)}>
          <NunitoText size="lg" weight={700}>

...
export function ApyCalculator({ isOpen, onDismiss, pool }: ConfirmationModalProps) {
  ...
  const remainingDays = Number((remainingTime / (24 * 60 * 60)).toFixed())
  ...
  const update = (value: number, _remainingDays: number) => {
    ...
    const projectedDerpEarningUSD = (+value / pool.tvlUSD) * remainingRewards

    const projectedDerpEarning = projectedDerpEarningUSD / pool.pricePerDerp
    ...
    const projectedLPEarningUSD = ((pool.feeUSD * value) / pool.volume) * _remainingDays

...
export default function GenesisNetworkFilter() {
  ...
  return (
    <DerpStyledMenu ref={node}>
      <DerpNetworkFilterOption
        onClick={toggleMenu}
        aria-label="genesisNetworkFilter"
        active={open}
        data-testid="genesis-pool-network-filter-selected"
      >
        <StyledMenuContent>
          {chainInfo && (
            <NetworkLabel>
          ...
          {DERPDEX_BACKEND_CHAIN_NAMES.map((network) => {
            ...
            return (
              <InternalLinkMenuItem
                key={network}
                data-testid={`genesis-pool-network-filter-option-${network.toLowerCase()}`}
                onClick={() => {
                  navigate(`/extended-genesis-pools/${network.toLowerCase()}`)
                  toggleMenu()
                }}
              >
                <NetworkLabel>

...
import tryParseCurrencyAmountLpInput from 'lib/utils/tryParseCurrencyAmountLpInput'
...
export function useV3DerivedMintInfo(
  ...
  // always returns the price with 0 as quoted token
  const quotePricesAtTicks = useMemo(() => {
    return {
      [Bound.LOWER]: getTickToPrice(token1, token0, ticks[Bound.UPPER]),
      [Bound.UPPER]: getTickToPrice(token1, token0, ticks[Bound.LOWER]),
    }
  }, [token0, token1, ticks])
  const { [Bound.LOWER]: lowerQuotePrice, [Bound.UPPER]: upperQuotePrice } = quotePricesAtTicks
  ...
  // amounts
  const independentAmount: CurrencyAmount<Currency> | undefined = tryParseCurrencyAmountLpInput(
    typedValue,

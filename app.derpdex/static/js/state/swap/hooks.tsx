...
import useParsedHashQueryString from 'hooks/useParsedHashQueryString'
...
export function useDerivedSwapInfo(state: SwapState, chainId: SupportedChainId | undefined): SwapInfo {
  ...
  const parsedAmount = useMemo(
    () => tryParseCurrencyAmountSwap(typedValue, (isExactIn ? inputCurrency : outputCurrency) ?? undefined),
  ...
  const inputError = useMemo(() => {
    ...
    if (!account) {
      inputError = (
        <Trans>
          <NunitoText>Connect Wallet</NunitoText>
        </Trans>
      )
    }
    ...
    if (!currencies[Field.INPUT] || !currencies[Field.OUTPUT]) {
      inputError = inputError ?? (
        <Trans>
          <NunitoText>Select a token</NunitoText>
        </Trans>
      )
    }
    ...
    if (!parsedAmount) {
      inputError = inputError ?? (
        <Trans>
          <NunitoText>Enter an amount</NunitoText>
        </Trans>
      )
    }
    ...
    if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
      inputError = (
        <Trans>
          <NunitoText>Insufficient {amountIn.currency.symbol} balance</NunitoText>
        </Trans>
      )
    }
...
export function useDefaultsFromURLSearch(): SwapState {
  ...
  const parsedQs = useParsedHashQueryString()

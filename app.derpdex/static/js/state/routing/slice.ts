...
import { AlphaRouter, ChainId } from '@derpdex/smart-order-router'
import { createApi, fetchBaseQuery, FetchBaseQueryError } from '@reduxjs/toolkit/query/react'
import { TradeType } from '@uniswap/sdk-core'
import { PRICE_IMPACT_LIMIT_FALLBACK } from 'constants/misc'
      ...
      async queryFn(args, _api, _extraOptions, fetch) {
        if (args.routerPreference === RouterPreference.API || args.routerPreference === RouterPreference.AUTO) {
          try {
            ...
            if (args.routerPreference === RouterPreference.AUTO) {
              const marketPriceImpact = tradeResult?.trade?.priceImpact
                ? computeRealizedPriceImpact(tradeResult.trade)
                : undefined

              const isV3HighPriceImpact = Number(marketPriceImpact?.toFixed(2)) > PRICE_IMPACT_LIMIT_FALLBACK

              if (!isV3HighPriceImpact) {

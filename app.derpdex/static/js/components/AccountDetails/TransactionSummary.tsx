...
function BridgeDepositSummary() {
  return <Trans>Bridge L1 to L2 Desposit Done</Trans>
}

function BridgeDepositTransferSummary() {
  return <Trans>Transfer Inscription Done</Trans>
}

function BridgeDepositMintSummary() {
  return <Trans>Wrap Inscription to ERC-20 tokens Done</Trans>
}

function BridgeWithdrawSummary() {
  return <Trans>Bridge L2 to L1 Withdraw Done</Trans>
}

function BridgeWithdrawBurnSummary() {
  return <Trans>Unwrap ERC-20 token to Inscription Done</Trans>
}

function ClaimDerpPointsSummary() {
  return <Trans>Successfully Claimed DerpPoints</Trans>
}

function RedeemDerpPointsSummary() {
  return <Trans>Successfully Redeemed DerpPoints</Trans>
}

function StakeLPSummary({ info }: { info: { positionId: string } }) {
  return <Trans>Staked PositionId: {info.positionId}</Trans>
}

function UnstakeLPSummary({ info }: { info: { positionId: string } }) {
  return <Trans>Unstaked PositionId: {info.positionId}</Trans>
}

function DerpBridgeDepositSummary() {
  return <Trans>Bridged to the other chain</Trans>
}

function DerpBridgeClaimSummart() {
  return <Trans>Claim Successful</Trans>
}
...
export function TransactionSummary({ info }: { info: TransactionInfo }) {
  switch (info.type) {
    ...
    case TransactionType.BRIDGE_DEPOSIT_TRANSFER:
      return <BridgeDepositTransferSummary />

    case TransactionType.BRIDGE_DEPOSIT_MINT:
      return <BridgeDepositMintSummary />

    case TransactionType.BRIDGE_WITHDRAW:
      return <BridgeWithdrawSummary />

    case TransactionType.BRIDGE_WITHDRAW_BURN:
      return <BridgeWithdrawBurnSummary />

    case TransactionType.BRIDGE_WITHDRAW_OPBNB:
      return <BridgeWithdrawSummary />
    case TransactionType.CLAIM_DERPPOINTS:
      return <ClaimDerpPointsSummary />
    case TransactionType.REDEEM_DERPPOINTS:
      return <RedeemDerpPointsSummary />

    case TransactionType.STAKE_LP:
      return <StakeLPSummary info={info} />
    case TransactionType.UNSTAKE_LP:
      return <UnstakeLPSummary info={info} />

    case TransactionType.DERP_BRIDGE_BRIDGE:
      return <DerpBridgeDepositSummary />

    case TransactionType.DERP_BRIDGE_CLAIM:
      return <DerpBridgeClaimSummart />
  }
}

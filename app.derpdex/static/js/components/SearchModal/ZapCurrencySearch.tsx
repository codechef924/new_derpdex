...
import { _SUPPORTED_INPUT_TOKEN_TO_ZAP } from 'pages/ZapToEarn/constants/general'
...
import { formatAnalyticsEventProperties } from './CurrencyList'
import ZapCurrencyList, { ZapCurrencyRow } from './CurrencyList/zap-currency-row'
import { PaddedColumn, Separator, ZapSearchInput } from './styleds'
...
export function ZapCurrencySearch({
  selectedCurrency,
  onCurrencySelect,
  ...
  const SUPPORTED_ERC20_TOKENS_TO_ZAP = useMemo(() => {
    if (chainId) {
      return _SUPPORTED_INPUT_TOKEN_TO_ZAP[chainId]
    } else {
      return []
    }
  }, [chainId])
  ...
  const filteredTokens: Token[] = useMemo(() => {
    return Object.values(defaultTokens)
      ...
      .filter((i) => SUPPORTED_ERC20_TOKENS_TO_ZAP.includes(i.wrapped.address.toLowerCase())) //! Filter for Zap To Earn only Supported
  }, [defaultTokens, debouncedQuery, SUPPORTED_ERC20_TOKENS_TO_ZAP])

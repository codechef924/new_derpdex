...
import { ReactComponent as DerpStar } from 'assets/svg/account-star.svg'
...
export default function AuthenticatedHeader({ account, openSettings }: { account: string; openSettings: () => void }) {
  ...
  const { data: portfolioBalances } = usePortfolioBalancesQuery({
    ...
    fetchPolicy: 'no-cache', // PrefetchBalancesWrapper handles balance fetching/staleness; this component only reads from cache
  })
            ...
            <AccountNamesWrapper>
              <ThemedText.SubHeader>
                <CopyText toCopy={ENSName ?? account}>
                  <GloriaText size="xxl">{ENSName ?? shortenAddress(account, 4, 4)}</GloriaText>
                </CopyText>
              </ThemedText.SubHeader>
              ...
              {ENSName && (
                <ThemedText.BodySmall color="textTertiary">
                  <CopyText toCopy={account}>
                    <GloriaText size="xxl">{shortenAddress(account, 4, 4)}</GloriaText>
                  </CopyText>

...
const Button = styled.button<{ isActive?: boolean; activeElement?: boolean }>`
  ...
  border: 2px solid ${({ theme, isActive }) => (isActive ? theme.derpPurp2 : theme.backgroundOutline)};
  border-radius: 50%;
  cursor: pointer;
  display: flex;
  outline: none;
...
const ButtonFill = styled.span<{ isActive?: boolean }>`
  background: ${({ theme, isActive }) => (isActive ? theme.derpGradientPrimary : theme.textTertiary)};
  border-radius: 50%;
  :hover {
    background: ${({ isActive, theme }) => (isActive ? theme.derpGradientPrimary : darken(0.05, theme.deprecated_bg4))};

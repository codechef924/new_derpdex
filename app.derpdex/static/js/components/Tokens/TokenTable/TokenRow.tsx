...
import QueryTokenLogoSubgraph from 'components/Logo/QueryTokenLogoSubgraph'
import { MouseoverTooltip } from 'components/Tooltip'
import { Chain } from 'graphql/data/Token'
import { SparklineMap, TopToken, TopTokenSubgraph } from 'graphql/data/TopTokens'
import {
  CHAIN_NAME_TO_CHAIN_ID,
  getTokenDetailsURL,
  getTokenDetailsURLSubgraph,
...
export const HEADER_DESCRIPTIONS: Record<TokenSortMethod, ReactNode | undefined> = {
  ...
  [TokenSortMethod.VOLUME]: (
    <Trans>Volume is the amount of the asset that has been traded on DerpDEX during the selected time frame.</Trans>
  ),
}
...
function TokenRowSubgraph({
  ...
  const rowCells = (
    <>
      ...
      {/* <PercentChangeCell data-testid="percent-change-cell" sortable={header}> TO BE ADDED
        {percentChange}
      </PercentChangeCell> */}
      <TvlCell data-testid="tvl-cell" sortable={header}>
        {tvl}
...
export function HeaderRow() {
  return (
    ...
    <TokenRowSubgraph
      ...
      tokenInfo={
        <Trans>
          <NunitoText>Token name</NunitoText>
        </Trans>
      }
      price={<HeaderCell category={TokenSortMethod.PRICE} />}
      percentChange={<HeaderCell category={TokenSortMethod.PERCENT_CHANGE} />}
...
interface LoadedRowSubgraphProps {
  tokenListIndex: number
  tokenListLength: number
  token: NonNullable<TopTokenSubgraph>
  chain: Chain
}

export const LoadedRowSubgraph = forwardRef((props: LoadedRowSubgraphProps, ref: ForwardedRef<HTMLDivElement>) => {
  const { tokenListIndex, tokenListLength, token, chain } = props
  const chainId = CHAIN_NAME_TO_CHAIN_ID[chain]

  // const delta = token.market?.pricePercentChange?.value
  // const arrow = getDeltaArrow(delta)
  const currency = useCurrency(token.token?.id, chainId)
  return (
    <div ref={ref} data-testid={`token-table-row-${token?.token?.symbol}`}>
      <StyledLink
        to={getTokenDetailsURLSubgraph(token, chain)}
        // onClick={() =>
      ...
      >
        <TokenRowSubgraph
          ...
          tokenInfo={
            <ClickableName>
              {currency ? <CurrencyLogo currency={currency} /> : <QueryTokenLogoSubgraph token={token} />}

              <TokenInfoCell>
                <TokenName data-cy="token-name">{token?.token?.name || ''}</TokenName>
          ...
          price={
            <ClickableContent>
              <PriceInfoCell>
                {formatUSDPrice(token?.priceUSD ? +token?.priceUSD : 0)}
          ...
          tvl={
            <ClickableContent>
              {formatNumber(token?.totalValueLockedUSD ? +token?.totalValueLockedUSD : 0, NumberType.FiatTokenStats)}
            </ClickableContent>
          }
          volume={
            <ClickableContent>
              {formatNumber(token?.volumeUSD ? +token?.volumeUSD : 0, NumberType.FiatTokenStats)}
            </ClickableContent>
          }
          // sparkLine={
          //   <SparkLine>

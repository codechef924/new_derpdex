...
function useConfirmModalState({
  ...
  const startSwapFlow = useCallback(async () => {
    ...
    if (allowance.state === AllowanceState.REQUIRED && !isAggregator) {
...
export default function ConfirmSwapModal({
  ...
  const disableAllowanceCheck = isAggregator ? { state: AllowanceState.ALLOWED } : null
    ...
    if (confirmModalState === ConfirmModalState.REVIEWING || showAcceptChanges) {
      return (
        <SwapModalFooter
          ...
          aggregatorPriceImpact={aggregatorPriceImpact}
        />
      )
    }
    return (
  ...
  const l2Badge = () => {
    if (isL2ChainId(chainId) && confirmModalState !== ConfirmModalState.REVIEWING) {
      ...
      return (
        <StyledL2Badge>
          <RowFixed data-testid="confirmation-modal-chain-icon" gap="8px">

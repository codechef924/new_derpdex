...
const CopyIcon = styled(Copy)`
  ...
  ${ClickableStyle}
  ${LinkStyle}
  stroke: ${({ theme }) => theme.white} !important;
`
...
export function CopyContractAddress({ address }: { address: string }) {
  ...
  const linkToExplorer = useMemo(() => {
    if (!pageChainId) return undefined

    return getExplorerLink(pageChainId, address, ExplorerDataType.ADDRESS)
  }, [pageChainId])
  ...
  return (
    <CopyContractAddressWrapper>
      <CopyAddressRow isClicked={isCopied}>
        <FullAddress onClick={() => window.open(linkToExplorer, '_blank')}>{address}</FullAddress>
        <TruncatedAddress onClick={() => window.open(linkToExplorer, '_blank')}>{truncated}</TruncatedAddress>
        <IconDiv onClick={copy}>
          <Copy size={14} />
        </IconDiv>
      </CopyAddressRow>

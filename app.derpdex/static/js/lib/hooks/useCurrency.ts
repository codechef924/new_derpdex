...
import { useTokenByDefinedChainId } from 'hooks/Tokens'
...
export function useTokenByChainId(tokenAddress: string | undefined, chainId: number): Token | null | undefined {
  const token = useTokenByDefinedChainId(tokenAddress, chainId)
  return useMemo(() => {
    // If the token is on another chain, we cannot fetch it on-chain, and it is invalid.
    return token
...
export function useCurrencyFromMap(
  ...
  const isNative = Boolean(
    (nativeCurrency && currencyId?.toUpperCase() === 'ETH') || (nativeCurrency && currencyId === 'BNB')
  )
